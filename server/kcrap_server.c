...
int main(int argc, char* argv[]) {
    ...
    krb5_klog_syslog(LOG_INFO, "Kcrap server started.");
...
static void mainloop(int sock, krb5_context context, krb5_principal sprinc, krb5_keytab keytab) {
	...
	krb5_klog_syslog(LOG_DEBUG, "Datagram of %d bytes received from address: %s.", ret, inet_ntoa(sa.sin_addr));
...
static int handle_auth_req(int sock, krb5_context context, krb5_auth_context auth_context, krb5_data *message, krb5_keyblock *keyblock, struct sockaddr_in *sa) {
    ...
    int my_cnt = 0;
    char chalstr[17];
    while (my_cnt < req.server_challenge.length) {
      sprintf(&chalstr[my_cnt*2], "%02x", (unsigned char)req.server_challenge.data[my_cnt]);
      my_cnt++;
    }

    krb5_klog_syslog(LOG_DEBUG, "Request for username '%s' from %s with challenge 0x%s.", req.principal.data, inet_ntoa(sa->sin_addr), chalstr);

    // int my_cnt = 0;
    // while (my_cnt < req.server_challenge.length) {
    //   printf("%02x", (unsigned char)req.server_challenge.data[my_cnt++]);
    // }
    // printf("\n");
    ...
    // Deleting key (md4(key21)) referenced by "extra" struct
    if (extra.length) {
      memset(extra.data, 0, extra.length);
      free(extra.data);
      krb5_klog_syslog(LOG_INFO, "Authentication for username '%s' from %s succeeded.", req.principal.data, inet_ntoa(sa->sin_addr));
    }
    else {
      krb5_klog_syslog(LOG_INFO, "Authentication for username '%s' from %s failed.", req.principal.data, inet_ntoa(sa->sin_addr));
    }
